...
class Invariants:
    ...
    def track(self, frame, event, arg):
        if event == "call" or event == "return":
            ...
            if function not in self.vars:
                self.vars[function] = {}
            if event not in self.vars[function]:
                self.vars[function][event] = {}
                
            if event == 'return':
                loc['ret'] = arg

            for key, val in loc.iteritems():
                if key not in self.vars[function][event]:
                    r = Range()
                    r.track(val)
                    self.vars[function][event][key] = r
                else:
                    self.vars[function][event][key].track(val)
                    ...
                    for var2, range2 in vars.iteritems():
                        if var != var2:
                            if (range.min == range2.min) and (range.max == range2.max):
                                comp = "=="
                            elif range.min < range2.min:
                                comp = "<="
                            else:
                                comp = ">="
                            s += "    assert " + var + " " + comp + " " + var2 + "\n"               
        return s
